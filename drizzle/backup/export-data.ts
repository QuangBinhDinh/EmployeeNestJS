...
async function exportData() {
  ...
  const outputFile = path.join(exportDir, `${dbName}_data_${timestamp}.sql`);

  try {
    console.log(`Exporting database data: ${dbName}...`);

    // Execute mysqldump via docker exec (data only)
    const command = `docker exec ${containerName} mysqldump -u ${dbUser} -p${dbPassword} --no-tablespaces --no-create-info ${dbName}`;
