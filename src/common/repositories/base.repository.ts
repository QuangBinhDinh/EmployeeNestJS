...
export abstract class BaseRepository<T extends MySqlTableWithColumns<any>> {
  ...
  public async txFindAll(
    ...
    const limit = pagination?.limit ?? DEFAULT_QUERY_LIMIT;
    const offset = pagination?.offset ?? 0;
    const result = await tx.select().from(this.table).limit(limit).offset(offset);
    return result as InferSelectModel<T>[];
  }
  ...
  public async txCount(tx: MySql2Database): Promise<number> {
    const result = await tx.select({ count: count() }).from(this.table);
    return result[0].count;
  }
  ...
  public async txFindOne(
    ...
    const result = await tx.select().from(this.table).where(eq(this.primaryKeyColumn, id)).limit(1);
    return result.length > 0 ? (result[0] as InferSelectModel<T>) : null;
  }
   ...
   */
  public async txCreate(
    tx: MySql2Database,
    data: InferInsertModel<T>,
  ): Promise<InferSelectModel<T>> {
    const newData = this.tranformDataInput(data);
    const result = await tx.insert(this.table).values(newData);
    const insertedId = result[0].insertId;
  ...
  public async update(
    ...
    const newData = this.tranformDataInput(data);
    await this.db.update(this.table).set(newData).where(eq(this.primaryKeyColumn, id)).limit(1);
    return await this.findOne(id);
  }
  /**
   * Update exactly one record within a transaction and return the updated row.
   */
  public async txUpdate(
    ...
    const newData = this.tranformDataInput(data);
    await tx.update(this.table).set(newData).where(eq(this.primaryKeyColumn, id)).limit(1);
    return await this.findOne(id);
  }
  ...
  public async txRemove(tx: MySql2Database, id: string | number): Promise<number> {
    if (!this.primaryKeyColumn) {
      throw new Error('Primary key not found for table');
    }
    const result = await tx.delete(this.table).where(eq(this.primaryKeyColumn, id));
    return result[0].affectedRows;
  }
  ...
  public async txFindByCondition(
    tx: MySql2Database,
    condition: Partial<InferSelectModel<T>>,
    pagination?: { limit: number; offset: number },
  ): Promise<InferSelectModel<T>[]> {
  ...
  public async txUpdateOneByCondition(
    tx: MySql2Database,
    condition: Partial<InferSelectModel<T>>,
    data: Partial<InferInsertModel<T>>,
  ): Promise<InferSelectModel<T> | null> {
    ...
    await tx.update(this.table).set(transformedData).where(whereClause).limit(1);

    const updated = await tx.select().from(this.table).where(whereClause).limit(1);
    return updated.length > 0 ? (updated[0] as InferSelectModel<T>) : null;
  }
