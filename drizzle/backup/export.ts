...
async function exportDatabase() {
  ...
  const outputFile = path.join(exportDir, `${dbName}_full_${timestamp}.sql`);

  try {
    console.log(`Exporting database: ${dbName} (structure + data)...`);

    // Execute mysqldump via docker exec
    const command = `docker exec ${containerName} mysqldump -u ${dbUser} -p${dbPassword} --no-tablespaces ${dbName}`;
